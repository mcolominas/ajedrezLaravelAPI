...
class TableroController extends Controller
{
    function ver(Request $request){
        ...
        if($userID1 != false && $userID2 != false){
            $partida = Partida::select("id", "turno")->where([["id_jugador_negro", $userID1],["id_jugador_blanco", $userID2]])->orWhere([["id_jugador_negro", $userID2],["id_jugador_blanco", $userID1]]);
            if($partida->count() > 0){
                $status = 1;
        ...
        if($status)
            return response(json_encode(["status" => $status, "tablero" => $tablero]), 200)->header('Content-Type', 'application/json')->header('Access-Control-Allow-Origin', '*');
        else
    ...
    function info(Request $request){
        ...
        $status = 0;
        if($userID1 != false && $userID2 != false){
            $partida = Partida::select("id_jugador_negro", "id_jugador_blanco")->where([["id_jugador_negro", $userID1],["id_jugador_blanco", $userID2]])->orWhere([["id_jugador_negro", $userID2],["id_jugador_blanco", $userID1]]);
            if($partida->count() > 0){
                $status = 1;
                if($partida->first()->toArray()["id_jugador_negro"] == $userID1)
                    $info = ["color" => "n"];
                else
                    $info = ["color" => "b"];
        ...
        if($status)
            return response(json_encode(["status" => $status, "partida" => $info]), 200)->header('Content-Type', 'application/json')->header('Access-Control-Allow-Origin', '*');
        else
            return response(json_encode(["status" => $status, "mensaje" => $mensaje]), 200)->header('Content-Type', 'application/json')->header('Access-Control-Allow-Origin', '*'); 
    }
